...
while 1:
    def get_args():
        ...
        smtp_options.add_argument("--ssl", dest="smtp_ssl", help="Connect to SMTP server via SSL",
                                  default=False)
        smtp_options.add_argument("--slow", action="store_true", dest="slow_send", default=False,
                                  help="Slow the sending")
    ...
    if __name__ == "__main__":
        ...
        try:
            print
            "Connecting to SMTP server at " + args.smtp_server + ":" + str(args.smtp_port)
